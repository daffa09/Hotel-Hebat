...
<div class="container-fluid bg-light" style="border-radius: 10px">
    ...
    <div class="image-hero mt-4" style="margin-left: 50px">
                    <img src="{{ asset('/storage/'.$gambar) }}" width="1200" height="500" class="align-items-center shadow p-3 mb-3 bg-body rounded" alt="Banner Utama">
    </div>

    <div class="d-flex justify-content-center">

        {{-- NOTE --}}
        {{-- untuk mengirim data menggunakan post jgn gunakan 'disable' pada form yang tidak ingin di rubah --}}
        {{-- Tetapi gunakan 'readonly' agar data tetap terikirim --}}

        <form class="d-flex justify-content-center" action="{{ url('/pemesanan/reservasi') }}" method="POST" enctype="multipart/form-data">
            @csrf

            <div class="form-group m-3">
                <label for="tgl_checkin">Tanggal Checkin</label>
                <input type="date" class="form-control" id="tgl_checkin" name="tgl_checkin" placeholder="Tanggal Checkin" value="{{ $tgl_checkin }}" min="{{ now()->format('Y-m-d') }}">
            </div>
            <div class="form-group m-3">
                <label for="tgl_checkout">Tanggal Checkout</label>
                <input type="date" class="form-control" id="tgl_checkout" name="tgl_checkout" placeholder="Tanggal Checkout" value="{{ $tgl_checkout }}" min="{{ now()->format('Y-m-d') }}">
            </div>
            <div class="form-group mt-2 m-3">
                <label for="id_tipe_kamar" class="form-label">Tipe Kamar</label>
                <select class="form-control" name="id_tipe_kamar" readonly>
                    <option value="{{ $id_tipe_kamar }}" selected>{{ $nama_kamar }}</option>
                </select>
            </div>
            <div class="form-group m-3" style="margin: 32px">
                <label for="jml_kamar">Jumlah Kamar</label>
                <input type="number" class="form-control" id="jml_kamar" name="jml_kamar" placeholder="Jumlah Kamar" value="{{ $jml_kamar }}">
            </div>
    </div>
    ...
    <div class="row">
        ...
        <div class="col-md-10 text-start py-3 text-center">
            ...
            <a href="/kamar" class="btn btn-success mt-2 col-md-12 mb-5">Kembali</a>
        </div>
    </div>
    </form>
